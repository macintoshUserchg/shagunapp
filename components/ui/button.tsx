...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-lg text-sm font-semibold transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default:
          "bg-[var(--accent)] text-white shadow hover:bg-[var(--accent)]/90 hover:-translate-y-0.5 hover:shadow-lg",
        destructive:
          "bg-red-500 text-white shadow-sm hover:bg-red-500/90 focus-visible:ring-red-500",
        outline:
          "border border-[var(--border)] bg-transparent shadow-sm hover:bg-[var(--surface-stat)] hover:border-[var(--muted)]",
        secondary:
          "bg-[var(--surface-stat)] text-[var(--foreground)] border border-[var(--border)] shadow-sm hover:bg-[var(--border)]",
        ghost: "hover:bg-[var(--surface-stat)] hover:text-[var(--foreground)]",
